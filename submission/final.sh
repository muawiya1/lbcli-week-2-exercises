...
# WRITE YOUR SOLUTION BELOW:
FIRST_OUTPUT_VALUE=$(bitcoin-cli -regtest decoderawtransaction "$BASE_TX" | jq -r '[.vout[0].value * 100000000] | .[0] | round')
...
# WRITE YOUR SOLUTION BELOW:
UTXO_TXID=$TXID
UTXO_VOUT_INDEX=$(bitcoin_cli -regtest decoderawtransaction "$BASE_TX" | jq -r '.vout[0].n')
check_cmd "UTXO vout selection" "UTXO_VOUT_INDEX" "$UTXO_VOUT_INDEX"

UTXO_VALUE=$(bitcoin-cli -regtest decoderawtransaction "$BASE_TX" | jq -r '.vout[0].value')
...
# WRITE YOUR SOLUTION BELOW:
VERIFY_RBF=$(echo "$DECODED_TX" | jq -r 'if .vin[0].sequence < 4294967294 then "true" else "false"' )
...
# WRITE YOUR SOLUTION BELOW:
CHILD_INPUTS='''[{"txid":"'$PARENT_TXID'", "vout":'$CHANGE_OUTPUT_INDEX', "sequence": 1}]'''
